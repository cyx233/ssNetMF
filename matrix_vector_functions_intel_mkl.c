...
mat *matrix_new(int nrows, int ncols)
{
    ...
    long long size_mat = nrows;
    size_mat = (long long)size_mat * (long long)ncols;
    M->d = (double *)calloc(size_mat, sizeof(double));
    M->nrows = nrows;
    M->ncols = ncols;
    return M;
}
...
{
    long long indexs = M->nrows;
    indexs = indexs * col_num;
    indexs = indexs + row_num;
    M->d[indexs] = val;
}
...
void initialize_random_large_matrix(mat *M)
{
    int i, j, m, n;
    double val;
    m = M->nrows;
    ...
    int maxint = INT_MAX;
    printf("debug in large random matrix,maxint:%d\n", maxint);
    long long res = m;
    res = res * n;
    int tmpn = res / maxint;
    if (res % maxint != 0)
    ...
    int k = n / tmpn;
    k++;
    printf("debug in large ,res:%lld,tmpn:%d,k:%d\n", res, tmpn, k);
    float *r;
    ...
    for (j = 0; j < tmpn; j++)
    {
        ...
        {
            rsize = res - m * j * k;
            r = (float *)malloc(rsize * sizeof(float));
        }
        else
        {
            rsize = m * k;
            r = (float *)malloc(m * k * sizeof(float));
        }
        printf("matrix size :%d ", rsize);
        vsRngGaussian(METHOD, stream, rsize, r, a, sigma);
#pragma omp parallel shared(M, rsize, r, m, k, j) private(i, val)
        {
#pragma omp parallel for
            for (i = 0; i < rsize; i++)
            {
...
*/
int compute_evals_and_evecs_of_symm_matrix(mat *S, vec *evals)
{
    //LAPACKE_dsyev( LAPACK_ROW_MAJOR, 'V', 'U', S->nrows, S->d, S->nrows, evals->d);
    int info = LAPACKE_dsyevd(LAPACK_COL_MAJOR, 'V', 'U', S->nrows, S->d, S->ncols, evals->d);
    return info;
}
...
void matrix_write2binary_file(mat *M, char *fname)
{
    int i, j;
    int m = M->nrows, n = M->ncols;
    vec *row_val = vector_new(n);
    size_t one = 1;
    FILE *fid = fopen(fname, "wb");
    for (i = 0; i < m; i++)
    {
        matrix_get_row(M, i, row_val);
        fwrite(row_val->d, sizeof(double), n, fid);
    }
    /*
    double nnz_val;
    for(i=0; i<m; i++){
        for(j=0; j<n; j++){
            nnz_val = matrix_get_element(M,i,j);
            fwrite(&nnz_val,sizeof(double),one,fid); //write nnz
        }
    }
    */
    fclose(fid);
    vector_delete(row_val);
}

void row_major_transform2column(mat *At, mat *A)
{
    long long i, j;
#pragma omp parallel shared(At, A) private(i, j)
    {
#pragma omp for
        for (i = 0; i < (A->nrows); i++)
        {
            for (j = 0; j < (A->ncols); j++)
            {
                At->d[j * (A->nrows) + i] = A->d[i * (A->ncols) + j];
            }
        }
    }
}

void writeMatrix2file(mat *M, char *filename)
{
    long long i, j, m = M->nrows, n = M->ncols;
    FILE *fid = fopen(filename, "wb");
    for (i = 0; i < m; i++)
    {
        for (j = 0; j < n; j++)
        {
            fprintf(fid, "%+.15lf ", M->d[j * m + i]);
        }
        fprintf(fid, "\n");
    }
    fclose(fid);
...
void modify_matrix_element(mat *M)
{
    long long i, j;
    long long len = M->nrows;
    len = len * (M->ncols);
    //printf("begin modify\n");

#pragma omp parallel shared(M, len) private(i)
    {
#pragma omp for
        for (i = 0; i < len; i++)
        {
            ...
            //M->d[i]=log(max(M->d[i]*para,1));
        }
    }

    /*
    printf("int max:%d\n",INT_MAX);
    int batch = INT_MAX/M->ncols;
    mat *M_row = matrix_new(batch,M->ncols);
    int vdlen = batch*M->ncols;
    int cat = ceil((1.0*M->nrows)/(1.0*batch)-1);
    cat++;
    for (i=0;i<cat;i++){
        if (i==cat-1){
            int vdlen = M->nrows-i*batch;
            vdLn(vdlen,M->d+i*batch,M->d+i*batch);
        }
        else
            vdLn(vdlen,M->d+i*batch,M->d+i*batch);
    }
    */
    ...
    for (i=0;i<cat;i++){
        if (i==cat-1){
            int rp = M->nrows - (i*batch);
            mat *M_row = matrix_new(rp,M->ncols);
            int vdlen = rp*M->ncols;
            int inds[rp];
            for (j=0;j<rp;j++)
                inds[j]=i*batch+j;
            matrix_get_selected_rows(M,inds,M_row);
            vdLn(vdlen,M_row->d,M_row->d);
            matrix_set_selected_rows(M,inds,M_row);
            matrix_delete(M_row);
        }
        else{
            int rp = batch;
    ...
    */
    ...
    //printf("end modify\n");
    //unsigned int accm = vmlGetMode() & VML_ACCURACY_MASK;
    //unsigned int denm = vmlGetMode() & VML_FTZDAZ_MASK;
    //unsigned int errm = vmlGetMode() & VML_ERRMODE_MASK;
    //printf("accm:%u,denm:%u,errm:%u\n",accm,denm,errm);
    //unsigned int oldmode = vmlSetMode(VML_LA | VML_FTZDAZ_ON | VML_ERRMODE_STDERR)
    //vmdLn((int)len,M->d,M->d,VML_LA | VML_FTZDAZ_MASK | VML_ERRMODE_STDERR);
    //vdLn(M->nrows*M->ncols,M->d,M->d);
    //note that Logx input <0 will get Qnan,need max(x,1) first
    //Log10
    //Log1p
    //Log2
    //Logb
...
void para_matrix_mult(mat *M, double para)
{
    long long i;
    long long len = M->nrows;
    len = len * (M->ncols);
...
//A+B=B
void matrix_matrix_add(mat *A, mat *B)
{
    double alpha = 1.0;
    long long n = A->nrows;
    n = n * (A->ncols);
    int incx = 1, incy = 1;
    cblas_daxpy(n, alpha, A->d, incx, B->d, incy);
    return;
}

long long caculate_nnz(mat *A)
{
    long long nnz = 0;
    long long rp = A->nrows, n = A->ncols;
    long long i, j;
#pragma omp parallel shared(A, rp, n) private(i, j) reduction(+ \
                                                              : nnz)
    {
#pragma omp for
        for (i = 0; i < rp; i++)
        {
            for (j = 0; j < n; j++)
            {
                if (fabs(matrix_get_element(A, i, j)) >= 1e-10)
                    nnz = nnz + 1;
            }
        }
    }
...
void Debug_matrix(mat *M)
{
    ...
    for (int i = 0; i < M->nrows; i++)
    {
        ...
        {
            if (!(matrix_get_element(M, i, j) == matrix_get_element(M, i, j)) || isinf(matrix_get_element(M, i, j)))
            {
                //printf("******%d******%d****%f**\n",i,j,matrix_get_element(M,i,j));
                idi = i;
                idj = j;
                ele = matrix_get_element(M, i, j);
                flag = 1;
                printf("Matrix exist nan or inf---->%d--%d--%f\n", idi, idj, ele);
            }
        }
    }
    if (flag)
    ...
    {
        printf("Matrix does not exist nan or inf element\n");
    }
}
